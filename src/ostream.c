...
static int _push_byte (sofab_ostream_t *ctx, uint8_t byte)
{
    ...
    {
        // Buffer full, flush if possible. This repeats sofab_ostream_flush's
        // drain-and-rewind rather than calling it, deliberately: calling it
        // stops _push_byte from being a leaf, and the toolchain then keeps it
        // out of line in every varint writer -- ~12% more instructions on a
        // typical encode to save a handful of bytes. Footprint is what this
        // corelib optimizes for, but not at that exchange rate.
        if (ctx->flush)
        {
...
/* Out of line only while the sequence writers share it. With sequences compiled
 * out _write_id_varint is its one caller, and keeping it separate would cost a
 * call frame to save nothing. */
SOFAB_NOINLINE
#endif
...
{
    // The buffer triple is installed exactly as a mid-stream swap installs it,
    // so init is that call plus the sink; the argument checks live there too.
    sofab_ostream_buffer_set(ctx, buffer, buflen, offset);
    ...
    int32_t element_count, int32_t element_size, int is_signed)
{
    ...
    for (int32_t i = 0; i < element_count; i++)
    {
        ...
        if (is_signed)
        {
            ...
            // shift on a 64-bit value costs a multi-instruction sequence on a
            // 32-bit target.
    ...
    sofab_fixlentype_t type)
{
    ...
    // contiguously, so the whole payload is one flat byte range — the per-element
    // loop would emit exactly these bytes, one call at a time.
