#!/usr/bin/env bash
#
# SofaBuffers C/C++ — guards README.md against CORELIB_PLAN §9.
#
...
#   5. §9.8 C/C++ ships two corelibs, so the comparison between them
...
#   9. Every in-document link still resolves to a heading, and the one heading
#      another repository links to by fragment is still there.
#
...
expected="## SofaBuffers C/C++ library
## Why this design
## Usage
## Memory handling
## Build & test
...
if [ -z "$badges" ]; then
    ...
    # Badge alt texts, in document order: "[![CI](…)](…)" -> "CI". Matched
    # case-insensitively and by substring, because a port may qualify a badge it
    # ships more than one of — this repo publishes "C coverage" for a C core with
    # a C++ wrapper over it. §9.2 asks for a coverage badge, not for that exact word.
    ...
    # …and in the order §9.2 lists them: CI, coverage, Docs. Rank each badge by
    # which of the three it matches, then require that ranking to be ascending.
    ranked="$(printf '%s\n' "$order" | awk '
        { lc = tolower($0) }
        lc ~ /coverage/ { print "2"; next }
        lc ~ /^ci$|continuous/ { print "1"; next }
        lc ~ /doc/ { print "3"; next }
    ')"
    if [ "$ranked" = "$(printf '%s\n' "$ranked" | sort -n)" ] && [ -n "$ranked" ]; then
        ok "§9.2 badges are in the CI / coverage / Docs order"
    else
        fail "§9.2 badges out of order: expected CI, coverage, Docs — got$(printf ' %s' $order)"
    fi
fi
...
if [ -z "$compare_heading" ]; then
    fail "§9.8 no subsection comparing the two C/C++ corelibs"
else
...
# ------------------------------------------ the anchor another repo links to

# corelib-cpp's README links here by fragment, at
# "#what-the-speed-difference-actually-is", to point a reader at the head-to-head
# numbers. A heading renamed on this side breaks that link silently — nothing in
# either repo's build resolves a cross-repo fragment.
if grep -qE '^#+ +What the speed difference actually is[[:space:]]*$' "$PROSE"; then
    ok "the heading corelib-cpp links to is intact"
else
    fail "the 'What the speed difference actually is' heading is gone or renamed; corelib-cpp's README links to #what-the-speed-difference-actually-is"
fi
