...
# whose capabilities are compiled out.
#
# On bare metal the answer is the library alone for every configuration but
# `full`, and that is deliberate rather than a limitation:
#   - the vector runner reads its JSON from a filesystem, so it is hosted-only;
#   - linking the unit-test image for an MCU is already at the edge of what fits
#     (it is why the AVR workflow currently runs on manual dispatch — see the
#     note at the top of build-gcc-avr.yaml), and `full-strict` adds several more
#     KB of utf8 tests and negative vectors on top. Nothing about that image is
#     configuration-specific, so building it four times would buy no coverage and
#     would make a size ceiling decide whether CI is green.
# `full` keeps building and inspecting it, exactly as these workflows did before
# the config axis existed.
# ---------------------------------------------------------------------------
sofab_config_target() {
    case "$2" in
        hosted)
            case "$1" in
                full|full-strict)      echo "all" ;;
                minimal|minimal-noobj) echo "sofab_vectortest" ;;
                *) echo "unknown configuration: $1" >&2; return 1 ;;
            esac
            ;;
        baremetal)
            case "$1" in
                full)                               echo "all" ;;
                full-strict|minimal|minimal-noobj)  echo "sofabuffers" ;;
                *) echo "unknown configuration: $1" >&2; return 1 ;;
            esac
            ;;
        *)
            echo "unknown profile: $2 (expected hosted or baremetal)" >&2
            return 1
            ;;
    esac
}
