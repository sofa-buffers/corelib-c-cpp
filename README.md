...
| `write()` of an `enum` array | Takes the container of scoped enums as stored — `std::vector<Gear>` or `InlineVector<Gear,N>` with `enum class Gear : int8_t` — and encodes it at the enum's declared underlying width, which is also what picks the signed or unsigned array wire type | The same, spelled the same, for the same bytes: one expression in generated code compiles against either port, and neither needs a converted copy |
