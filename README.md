...
through a callback-driven decoder. The same wire format is shared by C and C++.

### C: encoding with `ostream`

Init a `sofab_ostream_t` over a buffer, write fields, read the byte count:
...
For messages larger than the buffer, pass a flush callback. It fires whenever
the buffer fills (and on the final `sofab_ostream_flush()`):
...
Decoding is callback-driven: feed raw bytes and, inside the callback, bind each
...
The `object` API is the code-generator target for C: `sofabgen` emits a plain C
struct plus a **constant descriptor** that drives both encode and decode, so one
generic transcoder serves many messages and keeps flash small.
...
The C++ API is a thin, type-deducing wrapper. `write()` picks the encoding from
the argument type; calls chain through a `Result` carrying the first error:
...
callback, the C++ mirror of the C `ostream` flush path.
